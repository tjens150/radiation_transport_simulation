...
def gen_greens(alistiter,abins,ardep,nphotbin,adist,rbins,Etot,karr): #generate G_dep^inj
...
def gen_transf_int(z,k, transf, z_int,k_int,vrmsval):
    T_e_fin=np.zeros([z_int.size,k_int.size])
    T_e=np.zeros([z.size,k_int.size])
    for i in np.arange(z.size):
        tmp_int=interp1d(k,transf[i,:]/vrmsval[i],bounds_error=False,fill_value='extrapolate')
        T_e[i,:]=tmp_int(k_int)#this sqrt is the reason we took a minus sign in the first place
    for i in np.arange(k_int.size):
        tmp_int=interp1d(z,T_e[:,i],bounds_error=False,fill_value='extrapolate') 
        T_e_fin[:,i]=tmp_int(z_int)
    return T_e_fin

def interp_G_e_new(G_e,z,karr,tau,kint,transf,vrmsval,onthespot=False): #Interpolate G_e(\sqrt{2}k)
    this_transf=gen_transf_int(transf['z_inj'],transf['k'], transf['TF'],z_int(tau),kint,vrmsval)
    Del_e_fin=np.zeros([tau.size,kint.size])
    if onthespot:
        tmp_int=interp1d(z,np.sqrt(G_e[:,0]),bounds_error=False,fill_value='extrapolate')
        Del_e_fin=tmp_int(z_int(tau))[:,np.newaxis]*this_transf
        return Del_e_fin
    Del_e=np.zeros([z.size,kint.size])
    for i in np.arange(z.size):
        tmp_int=interp1d(karr,G_e[i,:],bounds_error=False,fill_value='extrapolate')
        Del_e[i,:]=tmp_int(np.sqrt(2)*kint)/np.sqrt(G_e[i,0]) #this sqrt is the reason we took a minus sign in the first place
    ...
    for i in np.arange(kint.size):
        ...
        Del_e_fin[:,i]=tmp_int(z_int(tau))*this_transf[:,i]
    return Del_e_fin
...
# def interp_G_e_new(G_e,z,karr,tau,kint,transf_int,onthespot=False): #Interpolate G_e(\sqrt{2}k)
#     Del_e_fin=np.zeros([tau.size,kint.size])
#     if onthespot: 
#         tmp_int=interp1d(z,np.sqrt(G_e[:,0]),bounds_error=False,fill_value='extrapolate')
#         Del_e_fin=tmp_int(z_int(tau))[:,np.newaxis]*transf_int(kint)[np.newaxis,:]
#         return Del_e_fin
#     Del_e=np.zeros([z.size,kint.size])
#     for i in np.arange(z.size):
#         tmp_int=interp1d(karr,G_e[i,:],bounds_error=False,fill_value='extrapolate')
...
def gen_A_B(tau,ell,mu,nu,gam,zcut=0):
    ...
    cut=z_int(tau)>=zcut
    for i in range(ell.size):
        A[:,:,i]=trap_int(quad_A[:,:,cut]*gam[i,cut],tau[cut],axis=2)
        B[:,:,i]=trap_int(quad_B[:,:,cut]*gam[i,cut],tau[cut],axis=2)
        if not (i % 10): print(('%s of %s' % (i,ell.size)))
...
with open('./J_ell_100_hi_times2.pkl',"rb") as f:
    J_ell=pickle.load(f)
...
with open('./Qsq.pkl',"rb") as f:
    Qsq=pickle.load(f)
with open('./QQt.pkl',"rb") as f:
    QQt=pickle.load(f)
with open('./Qtsq.pkl',"rb") as f:
    Qtsq=pickle.load(f)
with open('./Qt_scat.pkl',"rb") as f:
    Qt_scat=pickle.load(f)
with open('./Qt_trans.pkl',"rb") as f:
    Qt_trans=pickle.load(f)

with open('./C_ell_full_hi_times2.pkl',"rb") as f:
   C_ell=pickle.load(f)


with open('PBH_prog.txt','w') as f:
    print(('done reading in data'),file=f)
...
tau=np.exp(np.arange(np.log(eta_int(1400)),np.log(eta_int(900)),taulog))
tau=np.append(tau,np.exp(np.arange(np.log(eta_int(900)),np.log(eta_int(10)),20*taulog)))
tau=np.append(tau,np.arange(eta_int(10),eta_int(0),taulin))
...
def gen_Del_e_mbh(pbhmass,PI,fpbh,onthespot=False):
    if PI: PI_str='_PI'
    else: PI_str=''
    ...
    G_homo,G_k=gen_greens(alistiter,abins,ardep,nphotbin,adist,rbins,Etot,k_G)#(adep,ainj) (k,adep,ainj)
    ...
    with open('PBH_prog.txt','w') as f:
        print(('computing del_e %s' % pbhmass),file=f)
    ...
    Del_e=interp_G_e_new(G_e,z,k_G,tau,kint,transf,vrmsval,onthespot=onthespot) #Because of the minus sign, this is technically multiplied by an (i)
    return Del_e
...
#Cthet=np.swapaxes(gen_Cthet_arr(s,tau,k,kint[kcut],L),1,2) #l,k,tau swapped to l,tau,k
...
# with open('PBH_prog.txt','w') as f:
#     print(('computing Qt_trans'),file=f)
...
# del g_ell, g_ell_sp, g_til_ell, g_ell_s, g_ell_ms, l_mast
...
# with open('./C_ell_full.pkl',"wb") as f:
#     pickle.dump(C_ell,f)
#     pickle.dump(ell,f)

# with open('PBH_prog.txt','w') as f:
#     print(('finished cal_C_ell'),file=f)
...
#with open('F_png_local_10_22_times32.txt','w') as f:
...
def output_mbh(pbhmass,PI,fpbh,filename,onthespot=False,zcut=0):
    if PI: PI_str='_PI'
    else: PI_str=''
    if onthespot: ontstr='_on'
    else: ontstr=''
    
    Del_e=gen_Del_e_mbh(pbhmass,PI,fpbh,onthespot=onthespot)
    appstr='_mbh%s_f%s%s%s' % (pbhmass, fpbh,PI_str,ontstr) #naming scheme for data
    ...
    cal_A,cal_B=gen_A_B(tau,l_trsf,mu,nu,gam,zcut=zcut)
    ...
    tmp=(add_B)*QQt
    T_PBH=sym_A*Qsq+tmp*6#+np.moveaxis(tmp,(0,1,2,3),(0,2,1,3))+np.moveaxis(tmp,(0,1,2,3),(0,2,3,1))+np.moveaxis(tmp,(0,1,2,3),(2,0,1,3))+np.moveaxis(tmp,(0,1,2,3),(2,0,3,1))+np.moveaxis(tmp,(0,1,2,3),(2,3,0,1))
    ...
    with open(filename,'a') as f:
        f.write('%s %s %s %s %s ' % (pbhmass, float(onthespot), PI, fpbh, g_NL))
    ...
    # ax.set_xscale('log')
    # plt.savefig('g_bias'+appstr+'_on.pdf')
    del g_NL_quad


    tmp=(add_B)*QQt
    F_PBH=sym_A**2*Qsq+sym_A*(tmp)*12#+np.moveaxis(tmp,(0,1,2,3),(0,2,1,3))+np.moveaxis(tmp,(0,1,2,3),(0,3,1,2))+np.moveaxis(tmp,(0,1,2,3),(1,2,0,3))+np.moveaxis(tmp,(0,1,2,3),(1,3,0,2))+np.moveaxis(tmp,(0,1,2,3),(2,3,0,1)))
    tmp=add_B**2*Qtsq
    F_PBH+=tmp*6#+np.moveaxis(tmp,(0,1,2,3),(0,2,1,3))+np.moveaxis(tmp,(0,1,2,3),(0,3,1,2))+np.moveaxis(tmp,(0,1,2,3),(1,2,0,3))+np.moveaxis(tmp,(0,1,2,3),(1,3,0,2))+np.moveaxis(tmp,(0,1,2,3),(2,3,0,1))
    tmp=2*add_B*np.moveaxis(add_B,(0,1,2,3),(2,3,0,1))*Qt_trans
    F_PBH+=tmp*3#+np.moveaxis(tmp,(0,1,2,3),(0,3,1,2))+np.moveaxis(tmp,(0,1,2,3),(0,2,1,3))
    tmp=2*add_B*np.moveaxis(add_B,(0,1,2,3),(0,2,1,3))*Qt_scat
    F_PBH+=tmp*12#+np.moveaxis(tmp,(0,1,2,3),(1,2,3,0))+np.moveaxis(tmp,(0,1,2,3),(2,3,0,1))+np.moveaxis(tmp,(0,1,2,3),(3,0,1,2))+np.moveaxis(tmp,(0,1,2,3),(3,1,2,0))+np.moveaxis(tmp,(0,1,2,3),(1,0,2,3))+np.moveaxis(tmp,(0,1,2,3),(1,3,0,2))+np.moveaxis(tmp,(0,1,2,3),(2,3,1,0))+np.moveaxis(tmp,(0,1,2,3),(3,0,2,1))+np.moveaxis(tmp,(0,1,2,3),(2,1,0,3))+np.moveaxis(tmp,(0,1,2,3),(0,1,3,2))+np.moveaxis(tmp,(0,1,2,3),(0,3,2,1))
    del tmp, sym_A, add_B


    F_PBH_quad=F_PBH/denom/(4*3*2)/(float(fpbh)**2)
    ...
    # ax.set_xscale('log')
    # plt.savefig('f_pbh_var'+appstr+'_on.pdf')

    del F_PBH_quad

filename='results_mbh_10_22_times32_on_fixed.txt'
#filename='dummy'
with open(filename,'w') as f:
    f.write('mbh on_the_spot PI f_pbh g_bias f_var')
    f.write('\n')

zcut=0
mass_arr=[1,10,100,1000,10000]
f_arr=['1','1','1','1','1','1e-2','1e-2','1e-4','1e-4','1e-6']
for i in range(len(mass_arr)):
    pbhmass=mass_arr[i] #in solar masses
    for j in range(2):
        if j==0: PI=False #Photoionization or collisionally ionized?
        else: PI=True
        ont=True
        fpbh=f_arr[i*2+j] #fraction of f_pbh, these are hard coded for the time being

        output_mbh(pbhmass,PI,fpbh,filename,onthespot=ont,zcut=zcut)
