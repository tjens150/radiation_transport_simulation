...
fil='pickle/z%s_E%s_N%s_binned.pkl' % (znam,Enam,Nnam)
...
nphotbin={'z%s_E%s' % (znam,Enam):tnphotbin}
ardep={'z%s_E%s' % (znam,Enam):tardep}
adist={'z%s_E%s' % (znam,Enam):tadist}
zdist={'z%s_E%s' % (znam,Enam):tzdist}
rdist={'z%s_E%s' % (znam,Enam):trdist}
Edist={'z%s_E%s' % (znam,Enam):tEdist}
totnum={'z%s_E%s' % (znam,Enam):ttotnum}
astat={'z%s_E%s' % (znam,Enam):tastat}
Estat={'z%s_E%s' % (znam,Enam):tEstat}
#Nscatter={'z%s_E%s' % (znam,Enam):tNscatter}
...
#    ax.plot([],[],lw=0,label=r'($z_{\rm mean}$, $E_{\rm mean}$)')
    ...
    ax.set_xscale('log')
    fig.tight_layout()
    plt.savefig(fol+'/figures/'+nam+'clean.pdf')
    ax.set_yscale('log')
    fig.tight_layout()
