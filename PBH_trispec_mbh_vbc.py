...
# def interp_G_e_new(G_e,z,karr,tau,kint,transf_int): #Interpolate G_e(\sqrt{2}k)
#     Del_e=np.zeros([z.size,kint.size])
#     for i in np.arange(z.size):
#         tmp_int=interp1d(karr,G_e[i,:],bounds_error=False,fill_value='extrapolate')
...
def gen_A_B(tau,ell,mu,nu,gam,zcut=10):
...
with open('./J_ell_150.pkl',"rb") as f:
    J_ell=pickle.load(f)
...
l_max_g=150 # max ell taken for the collision term
L=np.arange(l_max_g)
...
def gen_Del_e_mbh(pbhmass,PI,fpbh):
    if PI: PI_str='_PI'
    else: PI_str=''
    ...
    transf_int=interp1d(transf['k'],transf['TF'][zind,:]/vrmsval[zind]) #this is the spatial part of the transfer function
    ...
    Del_e=interp_G_e_new(G_e,z,k_G,tau,kint,transf,vrmsval) #Because of the minus sign, this is technically multiplied by an (i)
    return Del_e
...
# with open('results_450.txt','w') as f:
...
def output_mbh(pbhmass,PI,fpbh,filename,zcut=10):
    if PI: PI_str='_PI'
    else: PI_str=''

    Del_e=gen_Del_e_mbh(pbhmass,PI,fpbh)
    appstr='_mbh%s_f%s%s' % (pbhmass, fpbh,PI_str) #naming scheme for data
    ...
    with open(filename,'a') as f:
        f.write('%s %s %s %s ' % (pbhmass, PI, fpbh, g_NL))
    ...
    # ax.set_xscale('log')
    # plt.savefig('f_pbh_var'+appstr+'.pdf')

    del F_PBH_quad

zcut=0
filename='results_mbh_new_150_vbc.txt'
with open(filename,'w') as f:
    f.write('mbh PI f_pbh g_bias f_var')
    f.write('\n')
...
for i in range(len(mass_arr)):
    ...
    for j in range(2):
        ...
        output_mbh(pbhmass,PI,fpbh,filename,zcut=zcut)
