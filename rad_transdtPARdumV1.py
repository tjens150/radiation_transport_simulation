...
def cross(Ei):
    return (3*me*sigT*(4*me + (2*Ei**2*(Ei + me))/(2*Ei + me)**2 - (4*me*(Ei + me)*np.arctanh(Ei/(Ei + me)))/Ei + Ei*np.log(1 + (2*Ei)/me)))/(8.*Ei**2)
...
def rotmat(thet,phi): #Rz.Ry
    return [[np.cos(phi)*np.cos(thet),-np.sin(phi),np.cos(phi)*np.sin(thet)],
            [np.sin(phi)*np.cos(thet),np.cos(phi),np.sin(phi)*np.sin(thet)],
    ...
    for ee in range(len(Eim)):
        ...
        for i in range(N):
            ...
            while Eistep*ai/astart > thresh*Ei:
                ...
                while pxstep < ppull:
                    ppull=np.random.uniform(0,1)
                    Eistep=Eistep*astep/(astep+dlna)
                    astep=astep+dlna
                    if 1/astep-1 < 10:
                        break
                    ptmp=nh0*astep**(1/2.)/(astart**3*np.sqrt(omegaM)*H0)*c*cross(Eistep)
        ...
        resNarr=None
        if rank == 0:
            resRlist=np.zeros([size,N])
            resafarr=np.zeros([size,N])
            resNarr=np.zeros([size,N], dtype='int')
            print(resNarr)
        comm.Gather(np.array(Rlist), resRlist, root=0)
        comm.Gather(np.array(afarr), resafarr, root=0)
        comm.Gather(np.array(Narr), resNarr, root=0)
        if rank == 0:
            fig,ax=plt.subplots(1,1)
            Rlist=resRlist.flatten()
            afarr=resafarr.flatten()
            Narr=resNarr.flatten()
            tit='kilpar'+str(int((1-thresh)*100))+'E'+str(Eim[ee])+'_'+str(Ntot)
            fil=tit+'.pkl'
            tit=tit+'.pdf'
            ...
            ax.set_ylabel('Percent of Photons (N='+str(Ntot)+')')
            weights=np.ones_like(Rlist)/float(len(Rlist))
            ax.hist(np.array(Rlist), bins=200, weights=weights)
            ax.set_xlim(0,500)
            plt.savefig('rhistzoom'+tit)
            plt.cla()
            ax.set_title('Ei='+str(Eim[ee])+'*M_e')
